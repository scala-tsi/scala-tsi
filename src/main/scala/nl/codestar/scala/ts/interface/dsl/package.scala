...
package object dsl {
  ...
  implicit def tuple2InterfaceMemberTs(member: (String, TypescriptType)): TSInterface.Member =
  ...
  implicit class TSInterfaceDSL(val interface: TSInterface) extends AnyVal {
    def +(member: TSInterface.Member): TSInterface = interface.copy(members = interface.members :+ member)
  ...
  implicit class TSITypeDSL[T](val tsiType: TSIType[T]) extends AnyVal {
    def +(member: TSInterface.Member): TSIType[T] = TSIType(tsiType.get + member)
    def ++(newMembers: Seq[TSInterface.Member]): TSIType[T] = TSIType(tsiType.get ++ newMembers)
  }

  def tsInterface[T](members: TSInterface.Member*)(implicit ct: Manifest[T]): TSIType[T] =
  ...
  def tsAlias[T, Alias](implicit tsType: TSType[Alias], ct: Manifest[T]): TSType[T] =
    tsAlias[T, Alias](ct.runtimeClass.getSimpleName)
  def tsAlias[T, Alias](name: String)(implicit tsType: TSType[Alias]): TSType[T] = TSType(TSAlias(name, tsType.get))
}
